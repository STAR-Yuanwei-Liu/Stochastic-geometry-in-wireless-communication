...
RL=75; % the radius of each cluster
...
for ii = 1 : length(SNR_dB)
    ...
    threshold_E(ii) = (alpha-2)*P_t(ii)*C_L*r_C^(-alpha)/ ( 2*pi*lambda*P_t(ii)*C_L*r_C^(2-alpha) + (alpha-2)*Noise );
    index_1 = zeros(1,m);
    index_2 = zeros(1,m);
    index_sum = zeros(1,m);
...
for n = 1:m
    ...
    A = [threshold2/(a_high-threshold2*a_low); threshold/a_low; threshold_E(ii)];
    A2 = [threshold2/(a_high-threshold2*a_low); threshold_E(ii)];
    Gamma = max(A2);
