...
r_C = 100; % connedted user distance to nearest user

Pt_dBm = -5:5:20; % transmit power dBm
...
for n = 1:m
    ...
    miu4 =@(x)  n*miu_c.*x.*Noise./(a_high-a_low.*x)/P_t(ii)/C_L;
    miu3 =@(x) pi*lambda.*hypergeom([-2/alpha,m],1-2/alpha,-n*miu_c.*x./m./(a_high-a_low.*x))-pi*lambda;
    GAMMA1 = min(a_high/a_low, threshold_E(ii)*a_high/(1+threshold_E(ii)*a_low));
    GAMMA2 = max(a_high/a_low, threshold_E(ii)*a_high/(1+threshold_E(ii)*a_low));
%     index = @(x) (-1)^(n+1)*nchoosek(m,n).*exp(-miu3(x).*r_C^2-miu4(x).*r_C^alpha);
    index = @(x) -(-1)^(n+1)*nchoosek(m,n).*exp(-miu1.*r_C^2-miu2.*r_C^alpha)+(-1)^(n+1)*nchoosek(m,n).*exp(-miu3(x).*r_C^2-miu4(x).*r_C^alpha);
    index2(n) = quadgk(@(x)  index(x)./(1+x), 0,GAMMA1);
end

CP_c(ii) = 1/log(2)*sum(index2);
toc
end

plot(SNR_dB,CP_c,'b-o');
hold on